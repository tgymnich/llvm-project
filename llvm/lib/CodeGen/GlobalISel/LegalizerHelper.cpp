...
LegalizerHelper::lowerFMinimum_FMaximum(MachineInstr &MI) {
  ...
  if (LI.isLegalOrCustom({CompOpcIeee, Ty})) {
    MinMax =
        MIRBuilder.buildInstr(CompOpcIeee, {Ty}, {Src0, Src1}, MI.getFlags())
            .getReg(0);
    MinMaxMustRespectOrderedZero = true;
  } else if (LI.isLegalOrCustom({CompOpc, Ty})) {
    MinMax = MIRBuilder.buildInstr(CompOpc, {Ty}, {Src0, Src1}, MI.getFlags())
                 .getReg(0);
  } else {
    // NaN (if exists) will be propagated later, so orderness doesn't matter.
    auto Comp = MIRBuilder.buildFCmp(CompPred, S1, Src0, Src1, MI.getFlags());
  ...
  if (!MI.getFlag(MachineInstr::FmNoNans) &&
    ...
    auto Comp =
        MIRBuilder.buildFCmp(CmpInst::FCMP_UNO, S1, Src0, Src1, MI.getFlags());
  ...
  if (!MinMaxMustRespectOrderedZero && !MI.getFlag(MachineInstr::FmNsz) &&
    ...
    auto IsZero = MIRBuilder.buildFCmp(CmpInst::FCMP_OEQ, S1, MinMax, Zero,
                                       MI.getFlags());
