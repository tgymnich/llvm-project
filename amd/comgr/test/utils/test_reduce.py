#!/usr/bin/env python3
"""Hermetic tests for utils/transpiler/reduce.py."""
...
SCRIPT = Path(__file__).resolve().parents[2] / "utils" / "transpiler" / "reduce.py"
SPEC = importlib.util.spec_from_file_location("transpiler_reduce", SCRIPT)
assert SPEC is not None and SPEC.loader is not None
transpiler_reduce = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = transpiler_reduce
SPEC.loader.exec_module(transpiler_reduce)
...
def write_bundle(
    ...
    bundle = {
        "format": transpiler_reduce.BUNDLE_FORMAT,
        "version": transpiler_reduce.BUNDLE_VERSION,
...
class DdminTests(unittest.TestCase):
    def test_finds_one_minimal_required_pair(self) -> None:
        ...
        result = transpiler_reduce.ddmin(list("abcdef"), interesting)
    ...
    def test_respects_minimum_size(self) -> None:
        result = transpiler_reduce.ddmin(
            [1, 2, 3],
...
class BundleValidationTests(unittest.TestCase):
    def test_rejects_malformed_metadata_list(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError,
                r"metadata\.cases must be a JSON list",
            ):
                transpiler_reduce.load_bundle(bundle)
    ...
    def test_rejects_duplicate_ids(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            path.write_text(json.dumps(bundle), encoding="utf-8")
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "duplicate code object id"
            ):
                transpiler_reduce.load_bundle(path)
    ...
    def test_bundle_paths_cannot_escape_bundle_directory(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(transpiler_reduce.ReducerError, "escapes"):
                transpiler_reduce.load_bundle(bundle_path)
            bundle["code_objects"][0]["path"] = str(outside)
            bundle_path.write_text(json.dumps(bundle), encoding="utf-8")
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "must be relative"
            ):
                transpiler_reduce.load_bundle(bundle_path)
    ...
    def test_rejects_non_finite_json(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(transpiler_reduce.ReducerError, "non-finite"):
                transpiler_reduce.load_json(path)
    ...
    def test_accepts_spaces_and_special_characters_in_paths(self) -> None:
        with tempfile.TemporaryDirectory(prefix="reduce spaces [") as name:
            ...
            candidate = transpiler_reduce.load_bundle(bundle)
            ...
            with tempfile.TemporaryDirectory() as output_name:
                ...
                transpiler_reduce.materialize_candidate(candidate, output)
    ...
    def test_materialization_rejects_changed_source_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "changed while materializing"
            ):
                transpiler_reduce.materialize_candidate(candidate, root / "candidate")
    ...
    def test_loads_inventory_nul_worklist_and_selector_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            candidate = transpiler_reduce.load_inputs(None, [], worklist, selection)
            self.assertEqual(
    ...
    def test_rejects_malformed_and_duplicate_nul_worklists(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "must end with NUL"
            ):
                transpiler_reduce.load_nul_worklist(worklist)
            encoded_path = os.fsencode(object_path.resolve())
            worklist.write_bytes(encoded_path + b"\0" + encoded_path + b"\0")
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "duplicate worklist path"
            ):
                transpiler_reduce.load_nul_worklist(worklist)
...
class PredicateTests(unittest.TestCase):
    def make_candidate(self, root: Path, value: bytes = b"interesting"):
        ...
        return transpiler_reduce.Candidate(
            (
                transpiler_reduce.CodeObject(
                    "id",
                    path.name,
                    path,
                    transpiler_reduce.sha256_file(path),
                ),
            ),
            {},
        )
    ...
    def make_runner(
        ...
        **overrides: object,
        ...
        values = {
            ...
            "timeout": 5.0,
            "work_root": root,
            "cache": transpiler_reduce.PredicateCache(None),
        }
        values.update(overrides)
        return transpiler_reduce.PredicateRunner(**values)
    ...
    def test_persistent_cache_survives_runner_recreation(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            self.assertEqual(cache["format"], transpiler_reduce.CACHE_FORMAT)
    ...
    def test_changed_identity_during_run_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "identity changed"
            ):
    ...
    def test_changed_identity_mode_invalidates_cached_result(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            candidate = self.make_candidate(root)
            runner = transpiler_reduce.PredicateRunner(
                [str(executable), "{input}"],
                0,
                1,
                5.0,
                root,
                transpiler_reduce.PredicateCache(None),
            )
    ...
    def test_capture_is_bounded(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            script = root / "predicate.py"
            script.write_text(
                f"print('x' * {transpiler_reduce.CAPTURE_LIMIT * 100})\n"
                "raise SystemExit(0)\n",
                encoding="utf-8",
            )
            runner = self.make_runner(root, script, [])
            ...
            self.assertLessEqual(
                len(result.stdout),
                transpiler_reduce.CAPTURE_LIMIT + len("\n<truncated>"),
            )
    ...
    def test_unknown_placeholder_fails_before_launch(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            root = Path(name)
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "unknown.*placeholder"
            ):
                transpiler_reduce.PredicateRunner(
    ...
    def test_input_placeholder_rejects_multi_object_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            two = transpiler_reduce.Candidate(
                one.code_objects
                + (
                    transpiler_reduce.CodeObject(
                        "second",
                        second_path.name,
                        second_path,
                        transpiler_reduce.sha256_file(second_path),
                    ),
                ),
                {},
            )
            ...
            with self.assertRaisesRegex(transpiler_reduce.ReducerError, "exactly one"):
                runner.evaluate(two)

    def test_nonfinite_timeouts_are_rejected_by_argument_parser(self) -> None:
        parser = transpiler_reduce.make_argument_parser()
        base_arguments = [
            "--code-object",
            "input.co",
            "--output",
...
class FakeElfToolTests(unittest.TestCase):
    ...
    def test_policy_only_allows_nonallocated_unprotected_sections(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            tools = transpiler_reduce.ElfSectionTools(
                str(readobj),
                str(objcopy),
                5.0,
                ["*"],
    ...
    def test_missing_tool_is_reported(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            root = Path(name)
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "could not find executable"
            ):
                transpiler_reduce.ElfSectionTools(
    ...
    def test_rejects_non_amdgpu_elf(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            obj.write_bytes(b"ELF")
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "requires an AMDGPU ELF"
            ):
    ...
    def test_objcopy_failure_does_not_replace_input(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            code_object = transpiler_reduce.CodeObject(
                "id", path.name, path, transpiler_reduce.sha256_file(path)
            )
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "intentional objcopy failure"
            ):
    ...
    def test_malformed_readobj_json_is_reported(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "malformed section JSON"
            ):
    ...
    def test_objcopy_output_must_remove_requested_sections(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "requested sections remain"
            ):
                tools.remove_sections(code_object, [".debug_info"])
            self.assertEqual(list(artifacts.iterdir()), [])
    ...
    def test_objcopy_output_must_preserve_unrequested_sections(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "unrequested sections disappeared"
            ):
                tools.remove_sections(code_object, [".debug_info"])
            self.assertEqual(list(artifacts.iterdir()), [])
...
class EndToEndTests(unittest.TestCase):
    def test_reduces_bundle_and_metadata_and_writes_reproducer(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            final, log = transpiler_reduce.run_from_arguments(args)
            self.assertEqual(
            ...
            self.assertEqual(log["format"], transpiler_reduce.LOG_FORMAT)
    ...
    def test_deterministic_output_and_log(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            for index in range(2):
                ...
                _, log = transpiler_reduce.run_from_arguments(args)
                logs.append(log)
                bundles.append(
    ...
    def test_offline_reference_manifest_differential(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            final, _ = transpiler_reduce.run_from_arguments(args)
            self.assertEqual(
                [item.path.read_bytes() for item in final.code_objects],
                [b"candidate-differs"],
            )
    ...
    def test_unsupported_section_format_publishes_nothing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                ...
                transpiler_reduce.run_from_arguments(args)
    ...
    def test_existing_output_is_never_overwritten(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "refusing to overwrite"
            ):
                transpiler_reduce.run_from_arguments(args)
    ...
    def test_cache_cannot_overwrite_external_bundle_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            external_metadata.write_text(
                json.dumps(
                    {
                        "format": transpiler_reduce.CACHE_FORMAT,
                        "version": transpiler_reduce.CACHE_VERSION,
                        "entries": {},
                    }
                ),
                encoding="utf-8",
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "overwrite reducer input"
            ):
                transpiler_reduce.run_from_arguments(args)
    ...
    def test_interruption_publishes_nothing_and_preserves_original(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            interesting = transpiler_reduce.PredicateResult("interesting", (0,), "", "")
            with mock.patch.object(
                transpiler_reduce.PredicateRunner,
                "evaluate",
    ...
    def test_publish_interruption_rolls_back_temporary_output(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with mock.patch.object(
                transpiler_reduce.os,
                "replace",
                side_effect=replace_or_interrupt,
            ):
                with self.assertRaises(KeyboardInterrupt):
                    transpiler_reduce.atomic_publish(
                        output,
                        candidate,
    ...
    def test_uninteresting_original_publishes_nothing(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(
                transpiler_reduce.ReducerError, "original input is not"
            ):
                transpiler_reduce.run_from_arguments(args)
            self.assertFalse(output.exists())
