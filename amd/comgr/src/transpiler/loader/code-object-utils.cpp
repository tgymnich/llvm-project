//===- code-object-utils.cpp - Transpiler ---------------------------------===//
//
...
static Error readKernelMetaAsUInt32(msgpack::MapDocNode &Map, StringRef Key,
  ...
  if (!Value || *Value < 0 || *Value > UINT32_MAX)
    return makeTranspilerError(formatv(
...
forEachKernelNode(msgpack::Document &Doc,
  ...
  if (!Root.isMap())
    return makeTranspilerError("AMDGPU metadata root is not a map");
  msgpack::DocNode *Kernels = findInMap(Root.getMap(), "amdhsa.kernels");
  if (!Kernels)
    return makeTranspilerError("AMDGPU metadata has no amdhsa.kernels");
  if (!Kernels->isArray())
    return makeTranspilerError("amdhsa.kernels is not an array");
  for (msgpack::DocNode &Kernel : Kernels->getArray()) {
    if (!Kernel.isMap())
      return makeTranspilerError("amdhsa.kernels entry is not a map");
...
static Expected<uint32_t> readMetadataMinorVersion(msgpack::Document &Doc) {
  ...
  if (!Version)
    return makeTranspilerError("AMDGPU metadata has no amdhsa.version");
  if (!Version->isArray() || Version->getArray().size() != 2)
    return makeTranspilerError("amdhsa.version is not a [major, minor] array");
  std::optional<int64_t> Major = nodeAsInt(Version->getArray()[0]);
  std::optional<int64_t> Minor = nodeAsInt(Version->getArray()[1]);
  if (!Major || !Minor)
    return makeTranspilerError("amdhsa.version major/minor is not an integer");
  if (*Major != 1)
    return makeTranspilerError(
...
Expected<CodeObjectInfo> CodeObjectInfo::create(MemoryBufferRef ElfData) {
  ...
  if (!ELFObj)
    return makeTranspilerError("code object is not a little-endian 64-bit ELF");
...
Expected<TextSection> CodeObjectInfo::textSection() const {
  ...
  if (!FoundText)
    return makeTranspilerError("textSection: .text section not found in ELF");
  return Result;
}
