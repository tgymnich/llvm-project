...
llvm::Expected<MCState> initMCState(StringRef TargetIsa) {
  ...
  //
  // The transpiler IR-raise pipeline does not currently exercise the MC
  // assembler (codegen runs through `llc`/`lld` on lifted IR, not
  // through this MCContext), so the abort does not fire on transpiler
