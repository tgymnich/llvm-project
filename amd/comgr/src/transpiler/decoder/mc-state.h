...
//===----------------------------------------------------------------------===//
//
// Per-ISA AMDGPU MC stack used by the transpiler raiser: target lookup, MC
...
#ifndef TRANSPILER_MC_STATE_H
#define TRANSPILER_MC_STATE_H
...
/// Owns the LLVM MC objects used by the transpiler raiser for one AMDGPU
...
/// transpiler-originated `TranspilerError` (Target lookup, MCSubtargetInfo /
...
/// the CPU name) or a `TranspilerError` when the Target rejects the ISA
/// string.
