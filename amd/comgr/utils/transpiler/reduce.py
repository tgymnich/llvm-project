#!/usr/bin/env python3
"""Reduce a transpiler corpus failure while preserving an external predicate.

The reducer treats the interestingness command as an argv vector. It never
passes user input through a shell.  See ``docs/TranspilerReduce.md`` for the
...
BUNDLE_FORMAT = "comgr-transpiler-reducer-bundle"
BUNDLE_VERSION = 1
LOG_FORMAT = "comgr-transpiler-reduction-log"
LOG_VERSION = 1
CACHE_FORMAT = "comgr-transpiler-reduction-cache"
CACHE_VERSION = 1
...
class Candidate:
    ...
    def digest(self) -> str:
        digest = hashlib.sha256()
        digest.update(b"comgr-transpiler-reducer-candidate-v1\0")
...
def run_from_arguments(args: argparse.Namespace) -> tuple[Candidate, dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="transpiler-reduce-") as temporary_name:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print(f"transpiler-reduce: filesystem error: {error}", file=sys.stderr)
        return 1
    print(
